...
typedef struct {
    rdb_update_result_t update_res;
    request_rec * r;
    long worker_end;
} request_result_t;
...
static void init_balancer_members(apr_pool_t *p, server_rec *s, proxy_balancer *balancer) {
    ...
    ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s, "Database created.");
...
/**
 * Before freeing up the resources used up by the request.
 */
static apr_status_t proxy_balancer_close_conn(void * request_result) {
    request_result_t * reqr = (request_result_t*)request_result;

    time_stat_t time_stat;
    perf_stat_t perf_stat;

    const char * xlb_proxy_start = apr_table_get(reqr->r->headers_in, AI_HK_PROXY_START);
    const char * xlb_worker_start = apr_table_get(reqr->r->headers_in, AI_HK_WORKER_START);
    const char * xlb_worker_id = apr_table_get(reqr->r->headers_in, AI_HK_WORKER_ID);
    const char * xlb_req_cmplx = apr_table_get(reqr->r->headers_in, AI_HK_REQUEST_COMPLEXITY);
    
    time_stat.proxy_start = atol(xlb_proxy_start);
    time_stat.worker_start = atol(xlb_worker_start);
    time_stat.worker_end = reqr->worker_end;
    time_stat.proxy_end = ai_current_timestamp();
    
    ai_get_perf_stat(&perf_stat, &time_stat);

    ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, NULL, "[OUT] @%s %s %s %ld %ld %ld %ld %ld %ld %ld %ld %d %u %u %u %u %d",
                 xlb_worker_id, xlb_req_cmplx, reqr->r->parsed_uri.query,
                 time_stat.proxy_start, time_stat.worker_start, time_stat.worker_end, time_stat.proxy_end,
                 perf_stat.total_time, perf_stat.queue_time, perf_stat.process_time, perf_stat.admin_time,
                 reqr->update_res.row_id, reqr->update_res.Q, reqr->update_res.request_count, reqr->update_res.prev_avg_pt, reqr->update_res.curr_avg_pt, reqr->update_res.delta_pt);
}

static void proxy_balancer_pre_read_request(request_rec *r, conn_rec *c) {
	char * proxy_start_str = (char*)apr_pcalloc(r->pool, 32*sizeof(char));
	long proxy_start = ai_current_timestamp();
	ai_timestamp_to_str(proxy_start_str, proxy_start);
	
	apr_table_setn(r->headers_in, AI_HK_PROXY_START, proxy_start_str);
}
                                      ...
                                      proxy_server_conf *conf, char **url)
{
    ...
    *worker = NULL;
    ...
    is_ai_active = strcmp((*balancer)->lbmethod->name, "ai") == 0;
    
    char * str_request_complexity = (char*)apr_palloc(r->pool, 16 * sizeof(char));
	char * str_worker_id = (char*)apr_palloc(r->pool, 16 * sizeof(char));
    char * worker_start_str  = (char*)apr_palloc(r->pool, 64*sizeof(char));
	int request_complexity = 5000;

   	
	if(is_ai_active) {
		if(r->parsed_uri.query) {
			request_complexity = ai_rdb_get_estimation(r->method, r->parsed_uri.query);
		} 
		
		if(r->parsed_uri.query && strcmp(r->parsed_uri.query, "load") == 0) {
			request_complexity = 0;
		}
		
		wid = ai_wlt_increment_minimum(request_complexity);
	}


	sprintf(str_request_complexity, "%d", request_complexity);
	sprintf(str_worker_id, "%u", wid);
	

    apr_table_setn(r->headers_in, AI_HK_WORKER_ID, str_worker_id);
    apr_table_setn(r->headers_in, AI_HK_REQUEST_COMPLEXITY, str_request_complexity);
    ...
    long worker_start = ai_current_timestamp();
    ai_timestamp_to_str(worker_start_str, worker_start);
    apr_table_setn(r->headers_in, AI_HK_WORKER_START, worker_start_str);
                                       ...
                                       proxy_server_conf *conf)
{
    ...
    request_result_t * request_res = (request_result_t*) apr_pcalloc(r->pool, sizeof(request_result_t));
	request_res->worker_end = ai_current_timestamp();
    request_res->r = r;
	unsigned int worker_id = 0;

    apr_pool_cleanup_register(r->pool, request_res, proxy_balancer_close_conn, apr_pool_cleanup_null);
	...
	const char * xlb_worker_start = apr_table_get(r->headers_in, AI_HK_WORKER_START);
	const char * xlb_worker_id = apr_table_get(r->headers_in, AI_HK_WORKER_ID);
	const char * xlb_req_cmplx = apr_table_get(r->headers_in, AI_HK_REQUEST_COMPLEXITY);
	
	if(xlb_worker_id &&
	   xlb_req_cmplx) {
		worker_id = (unsigned int)atoi(xlb_worker_id);
		int complexity = atoi(xlb_req_cmplx);

        ai_wlt_decrement_worker_load(worker_id, complexity);
	}

	if(xlb_worker_start) {
		if(r->parsed_uri.query) {
            long worker_start = atol(xlb_worker_start);
            long pt = request_res->worker_end - worker_start;
            
            if(strcmp(balancer->lbmethod->name, "ai") == 0) {
				ai_rdb_update(r->method, r->parsed_uri.query, pt, &(request_res->update_res));
            } else {
                request_res->update_res.result = APR_SUCCESS;
			}

            if(request_res->update_res.result != APR_SUCCESS) {
                ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, "Failed to update RDB row");
            }
		}
	}
...
static void ap_proxy_balancer_register_hook(apr_pool_t *p)
{
    ...
    ap_hook_pre_read_request(proxy_balancer_pre_read_request, NULL, NULL, APR_HOOK_FIRST);
